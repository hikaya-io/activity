...
class Formset(LayoutObject):
    ...
    def render(self, form, form_style, context, template_pack=TEMPLATE_PACK):
        ...
        return render_to_string(self.template, {'wrapper': self, 'formset': self.formset_name_in_context,
...
class ProjectAgreementForm(forms.ModelForm):
    ...
    me_reviewed_by_date = forms.DateField(
        label="M&E Reviewed by Date", widget=DatePicker.DateInput(), required=False)
    ...
    effect_or_impact = forms.CharField(
        help_text="Please do not include outputs and keep less than 120 words. Describe the logic that will " +
                  "link this project/activity to the proposed desired outcome/goal. Note any assumptions that " +
                  "are critical in this logic chain.", widget=forms.Textarea, required=False)
    justification_background = forms.CharField(
        help_text="As someone would write a background and problem statement in a proposal, this should be " +
                  "described here. What is the situation in this community where the project is proposed and what " +
                  "is the problem facing them that this project will help solve", widget=forms.Textarea, required=False)
    justification_description_community_selection = forms.CharField(
        help_text="How was this community selected for this project. It may be it was already selected as part " +
                  "of the project (like CDP-2, KIWI-2), but others may need to describe, out of an entire cluster, " +
                  "why this community? This can't be just 'because they wanted it', or 'because they are poor.' " +
                  "It must refer to a needs assessment, some kind of selection criteria, maybe identification by the " +
                  "government, or some formal process.", widget=forms.Textarea, required=False)
    description_of_project_activities = forms.CharField(
        help_text="Briefly describe the day to day work you plan to complete in order to accomplish this project. " +
                  "Include rationale for budget, scope, timeframe as well as staff and stakeholders that will be " +
                  "necessary to seeing this project is effectively implemented. Site any documentation/monitoring " +
                  "efforts that you'll need to do before completion.", widget=forms.Textarea, required=False)
    description_of_government_involvement = forms.CharField(
        help_text="This is an open-text field for describing the project. It does not need to be too long, but this " +
                  "is where you will be the main description and the main description that will be in the database. " +
                  "Please make this a description from which someone can understand what this project is doing. " +
                  "You do not need to list all activities, such as those that will appear on your benchmark list. " +
                  "Just describe what you are doing. You should attach technical drawings, technical appraisals, " +
                  "bill of quantity or any other appropriate documentation", widget=forms.Textarea, required=False)
    documentation_government_approval = forms.CharField(
        help_text="Check the box if there IS documentation to show government request for or approval of the " +
                  "project. This should be attached to the proposal, and also kept in the program file.",
        widget=forms.Textarea, required=False)
    description_of_community_involvement = forms.CharField(
        help_text="How the community is involved in the planning, approval, or implementation of this project " +
                  "should be described. Indicate their approval (copy of a signed MOU, or their signed Project " +
                  "Prioritization request, etc.). But also describe how they will be involved in the implementation " +
                  " - supplying laborers, getting training, etc.", widget=forms.Textarea, required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        <div class='panel panel-default'>
          ...
          {% if getBenchmark %}
              ...
              <table class="table">
                ...
                <tr>
                    ...
                    <td>{{ item.budget }}</td>
                    <td><a class="benchmarks" data-toggle="modal" data-target="#myModal" 
                    href='/workflow/benchmark_update/{{ item.id }}/'>Edit</a> | <a class="benchmarks" 
                        href='/workflow/benchmark_delete/{{ item.id }}/' data-toggle="modal" data-target="#myModal">
                        Delete</a></td>
                </tr>
                {% endfor %}
              </table>
          ...
          <div class="panel-footer">
            <a class="benchmarks" data-toggle="modal" data-target="#myModal" href="/workflow/benchmark_add/{{ pk }}">
                Add Component</a>
          </div>
        </div>
                ...
                Tab('Budget',
                    Fieldset(
                        'Budget',
                        PrependedAppendedText('total_estimated_budget', '$', '.00'), PrependedAppendedText(
                            'mc_estimated_budget', '$', '.00'),
                        AppendedText('local_total_estimated_budget', '.00'), AppendedText(
        ...
        <div class='panel panel-default'>
          ...
          {% if getBudget %}
                ...
                <tr>
                    ...
                    <td><a class="output" data-toggle="modal" data-target="#myModal" 
                    href='/workflow/budget_update/{{ item.id }}/'>Edit</a> | <a class="output" 
                    href='/workflow/budget_delete/{{ item.id }}/' data-toggle="modal" data-target="#myModal" >Delete</a>
                </tr>
                {% endfor %}
          ...
          <div class="panel-footer">
            <a class="output" data-toggle="modal" data-target="#myModal" href="/workflow/budget_add/{{ pk }}">
            Add Budget Contribution</a>
                ...
                Tab('Justification and Description',
                    ...
                    Fieldset(
                        'Justification',
                        Field('effect_or_impact', rows="4", css_class='input-xlarge',
                ...
                <table class="table">
                    {% for item in getQuantitative %}
                        {% ifchanged item.indicator.id %}
                            <tr>
                                <td><a href="/indicators/indicator_update/{{ item.indicator_id }}">
                                    {{ item.indicator}}<a/></td>
                            </tr>
                        {% endifchanged %}
                    {% endfor %}
          ...
          {% if getMonitor %}
              ...
              <table class="table">
                ...
                <tr>
                    ...
                    <td>{{ item.type}}</td>
                    <td><a class="monitoring" data-toggle="modal" data-target="#myModal" 
                    href='/workflow/monitor_update/{{ item.id }}/'>Edit</a> | <a class="monitoring" 
                    href='/workflow/monitor_delete/{{ item.id }}/' data-toggle="modal" data-target="#myModal">Delete</a>
                </tr>
                {% endfor %}
              </table>
          {% endif %}
          <div class="panel-footer">
            <a class="monitoring" data-toggle="modal" data-target="#myModal" href="/workflow/monitor_add/{{ pk }}">
                ...
                Tab('Approval',
                    Fieldset('Approval',
                             ...
                             'finance_reviewed_by', 'finance_reviewed_by_date', 'me_reviewed_by', 'me_reviewed_by_date',
        ...
        <div class='panel panel-default'>
          ...
          {% if getDocuments %}
              ...
              <table class="table">
                ...
                <tr>
                    ...
                    <td>{{ item.description}}</td>
                    <td><a class="monitoring" data-toggle="modal" data-target="#myModal" 
                    href='/workflow/documentation_agreement_update/{{ item.id }}/{{ pk }}/'>Edit</a> | 
                        <a class="monitoring" href='/workflow/documentation_agreement_delete/{{ item.id }}/' 
          ...
          {% endif %}
          <div class="panel-footer">
            <a onclick="newPopup('/workflow/documentation_list/0/{{ pk }}','Add New Documentation'); return false;" 
...
class ProjectAgreementSimpleForm(forms.ModelForm):
    ...
    class Meta:
        ...
        exclude = ['create_date', 'edit_date', 'account_code', 'lin_code', 'mc_estimated_budget',
                   ...
                   'finance_reviewed_by', 'me_reviewed_by', 'exchange_rate', 'exchange_rate_date',
    ...
    effect_or_impact = forms.CharField(
        help_text="Please do not include outputs and keep less than 120 words. Describe the logic that will link " +
                  "this project/activity to the proposed desired outcome/goal. Note any assumptions that are " +
                  "critical in this logic chain.", widget=forms.Textarea, required=False)
    justification_background = forms.CharField(
        help_text="As someone would write a background and problem statement in a proposal, this should be " +
                  "described here. What is the situation in this community where the project is proposed and " +
                  "what is the problem facing them that this project will help solve",
        widget=forms.Textarea, required=False)
    justification_description_community_selection = forms.CharField(
        help_text="How was this community selected for this project. It may be it was already selected as part " +
                  "of the project (like CDP-2, KIWI-2), but others may need to describe, out of an entire " +
                  "cluster, why this community? This can't be just 'because they wanted it', or 'because they " +
                  "are poor.' It must refer to a needs assessment, some kind of selection criteria, maybe " +
                  "identification by the government, or some formal process.", widget=forms.Textarea, required=False)
    description_of_project_activities = forms.CharField(
        help_text="Briefly describe the day to day work you plan to complete in order to accomplish this " +
                  "project. Include rationale for budget, scope, timeframe as well as staff and stakeholders " +
                  "that will be necessary to seeing this project is effectively implemented. Site any " +
                  "documentation/monitoring efforts that you'll need to do before completion.",
        widget=forms.Textarea, required=False)
    description_of_government_involvement = forms.CharField(
        help_text="This is an open-text field for describing the project. It does not need to be too long, but " +
                  "this is where you will be the main description and the main description that will be in the " +
                  "database.  Please make this a description from which someone can understand what this project " +
                  "is doing. You do not need to list all activities, such as those that will appear on your " +
                  "benchmark list. Just describe what you are doing. You should attach technical drawings, " +
                  "technical appraisals, bill of quantity or any other appropriate documentation",
        widget=forms.Textarea, required=False)
    documentation_government_approval = forms.CharField(
        help_text="Check the box if there IS documentation to show government request for or approval of the " +
                  "project. This should be attached to the proposal, and also kept in the program file.",
            ...
            <tbody>
                {% if getBudget %}
                    ...
                    <tr>
                        ...
                        <td>{{ item.description_of_contribution}}</td>
                        <td>{{ item.proposed_value}}</td>
                        <td><a class="output" href='/workflow/budget_update/{{ item.id }}/'>Edit</a> | 
                            <a class="output" href='/workflow/budget_delete/{{ item.id }}/'>Delete</a>
                    </tr>
                    {% endfor %}
                {% endif %}
            </tbody>
...
class ProjectCompleteCreateForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs['initial'].get('short'):
            fieldset = Fieldset('Program', 'program2', 'program', 'project_agreement2', 'project_agreement',
                                'activity_code', 'office', 'sector', 'project_name', 'estimated_budget', 'site',
                                'stakeholder',
                                )
        else:
            fieldset = Fieldset('Program', 'program2', 'program', 'project_agreement', 'project_agreement2',
                                'activity_code', 'account_code', 'lin_code', 'office', 'sector', 'project_name',
        ...
        self.fields['program'].widget = forms.HiddenInput()
        self.fields['program2'].label = "Program"
        self.fields['project_agreement2'].initial = "%s - %s" % (kwargs['initial'].get(
            'office'),  kwargs['initial'].get('project_name', 'No project name'))
...
class ProjectCompleteForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            TabHolder(
                Tab('Executive Summary',
                    Fieldset('', 'program', 'program2', 'project_agreement', 'project_agreement2', 'activity_code',
                             'account_code', 'lin_code', 'office', 'sector', 'project_name', 'project_activity',
                             'site', 'stakeholder',
                             ),
                    Fieldset(
                        'Dates',
        ...
        <div class='panel panel-default'>
          ...
          {% if getBenchmark %}
              ...
              <table class="table">
                ...
                <tr>
                    ...
                    <td>{{ item.cost}}</td>
                    <td><a class="benchmarks" data-toggle="modal" data-target="#myModal" 
                    href='/workflow/benchmark_complete_update/{{ item.id }}/'>Edit</a> | 
                    <a class="benchmarks" href='/workflow/benchmark_complete_delete/{{ item.id }}/' 
          ...
          <div class="panel-footer">
            ...
            href="/workflow/benchmark_complete_add/{{ id }}/?is_it_project_complete_form=true">Add Component</a>
          </div>
        </div>
                ...
                Tab('Budget',
                    Fieldset(
                        ...
                        'actual_cost_date', 'budget_variance', 'explanation_of_variance',
                        PrependedAppendedText('total_cost', '$', '.00'),
                        PrependedAppendedText('agency_cost', '$', '.00'),
                        AppendedText('local_total_cost', '.00'),
                        AppendedText('local_agency_cost', '.00'), 'exchange_rate', 'exchange_rate_date',
                    ),
        ...
        <div class='panel panel-default'>
          ...
          {% if getBudget %}
                ...
                <tr>
                    ...
                    <td>{{ item.proposed_value}}</td>
                    <td><a class="output" data-toggle="modal" data-target="#myModal" 
                    href='/workflow/budget_update/{{ item.id }}/'>View</a> | <a class="output" 
                    href='/workflow/budget_delete/{{ item.id }}/' data-toggle="modal" data-target="#myModal" >Delete</a>
                </tr>
                {% endfor %}
          {% endif %}
          ...
          <div class="panel-footer">
            ...
            href="/workflow/budget_add/{{ id }}/?is_it_project_complete_form=true">Add Budget Contribution</a>
          </div>
        </div>
                                """),
                             ),
        ...
        <div class='panel panel-default'>
            ...
            {% endif %}
                    ...
                    Fieldset(
                        '', AppendedText(
                            'progress_against_targets', '%'), 'beneficiary_type', 'direct_beneficiaries',
                        'average_household_size', 'indirect_beneficiaries', 'capacity_built', 'quality_assured',
        ...
        <div class='panel panel-default'>
          ...
          {% endif %}
          <div class="panel-footer">
            <a onclick="newPopup('/workflow/documentation_list/0/{{ id }}','Add New Documentation'); 
            return false;" href="#" class="btn btn-sm btn-info">Add New Documentation</a>
          </div>
        </div>
                         """),
                ),
...
class ProjectCompleteSimpleForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        <div class='panel panel-default'>
            ...
            <div class="panel-footer">
                <a class="benchmarks" data-toggle="modal" data-target="#myModal" 
                href="/workflow/benchmark_complete_add/{{ id }}/?is_it_project_complete_form=true" id="btn_bench">
                Add Component</a>
            </div>
        </div>
                        """),
                ...
                Tab('Budget',
                    Fieldset('',
                             PrependedAppendedText('estimated_budget', '$', '.00'), PrependedAppendedText(
                                 'actual_budget', '$', '.00')
                             ),
                    Fieldset("Other Budget Contributions:",
                             Div(
        ...
        <div class='panel panel-default'>
                    ...
                    {% if getBudget %}
                        ...
                        {% for item in getBudget %}
                            <tr>
                                ...
                                <td>{{ item.proposed_value}}</td>
                                <td><a class="output" data-toggle="modal" data-target="#myModal" 
                                href='/workflow/budget_update/{{ item.id }}/'>View</a> | 
                                <a class="output" href='/workflow/budget_delete/{{ item.id }}/' 
                                data-toggle="modal" data-target="#myModal" >Delete</a>
                            </tr>
                        {% endfor %}
                    {% endif %}
            ...
            <div class="panel-footer">
                <a class="output" data-toggle="modal" data-target="#myModal" 
                href="/workflow/budget_add/{{ pk }}/?is_it_project_complete_form=true">Add Budget Contribution</a>
            </div>
        </div>
                            """),
                             ),
        ...
        <div class='panel panel-default'>
            ...
            {% endif %}
                    ...
                    Fieldset('',
                             AppendedText(
                                 'progress_against_targets', '%'), 'beneficiary_type', 'capacity_built',
                             'quality_assured', 'issues_and_challenges', 'lessons_learned'
                             ),
                    ),
                Tab('Approval',
                    Fieldset('Approval',
                             'approval', 'approved_by', Field(
                                 'approval_remarks', rows="3", css_class='input-xlarge')
                             ),
                    ),
            ),
            FormActions(
        ...
        <div class='panel panel-default'>
            ...
            {% endif %}
            <div class="panel-footer">
                <a onclick="newPopup('/workflow/documentation_list/0/{{ id }}','Add New Documentation'); return false;" 
                href="#" class="btn btn-sm btn-info">Add New Documentation</a>
            </div>
        </div>
                    """),
...
class SiteProfileForm(forms.ModelForm):
    ...
    map = forms.CharField(widget=GoogleMapsWidget(
        attrs={'width': 700, 'height': 400, 'longitude': 'longitude', 'latitude': 'latitude',
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            TabHolder(
                ...
                Tab('Location',
                    Fieldset('Places',
                             'country', 'province', 'district', 'admin_level_three', 'village', Field(
                                 'latitude', step="any"), Field('longitude', step="any"),
                             ),
                    Fieldset('Map',
                             'map',
                             ),
                ...
                Tab('Demographic Information',
                    Fieldset('Households',
                             'total_num_households', 'avg_household_size', 'male_0_5', 'female_0_5', 'male_6_9',
                             'female_6_9', 'male_10_14', 'female_10_14', 'male_15_19', 'female_15_19', 'male_20_24',
                             'female_20_24', 'male_25_34', 'female_25_34', 'male_35_49', 'female_35_49', 'male_over_50',
                             'female_over_50', 'total_population',
                             ),
                    Fieldset('Land',
                             'classify_land', 'total_land', 'total_agricultural_land', 'total_rainfed_land',
                             'total_horticultural_land', 'populations_owning_land', 'avg_landholding_size',
...
class DocumentationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            'name', FieldWithButtons('url', StrictButton("gdrive", onclick="onApiLoad();")), Field(
        ...
        if "benchmark_complete" in self.request.path:
            self.helper.layout = Layout(
                Field('description', rows="3",
                      css_class='input-xlarge'), 'site', 'est_start_date', 'est_end_date',
                Field('actual_start_date', css_class="act_datepicker",
                      id="actual_start_date_id"),
                Field('actual_end_date', css_class="act_datepicker",
                      id="actual_end_date_id"), 'budget', 'cost', 'agreement', 'complete',
            )
        else:
            self.helper.layout = Layout(
                Field('description', rows="3",
                      css_class='input-xlarge'), 'site', 'est_start_date', 'est_end_date', 'budget', 'agreement',
            )
...
class StakeholderForm(forms.ModelForm):
    ...
    class Meta:
        model = Stakeholder
        # fields = ['contact', 'country', 'approved_by', 'filled_by', 'sectors',
        #           'formal_relationship_document', 'vetting_document', ]
        exclude = ['create_date', 'edit_date']
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            TabHolder(
                Tab('Details',
                    Fieldset('Details',
                             'name', 'type', 'contact',
                             HTML("""<a onclick="window.open('/workflow/contact_add/%s/0/').focus();">
                                    Add New Contact</a>""" % pkval), 'country', 'sectors',
                             PrependedText('stakeholder_register', ''), 'formal_relationship_document',
        ...
        self.fields['formal_relationship_document'].queryset = Documentation.objects.filter(
            program__country__in=countries)
        self.fields['vetting_document'].queryset = Documentation.objects.filter(
            program__country__in=countries)
