...
urlpatterns = [
    ...
    re_path(r'^(?P<id>\w+)/([0-9]+)/$', DefaultCustomDashboard,
            name='default_custom_dashboard'),

    # project status
    re_path(r'^(?P<id>[0-9]+)/(?P<status>[\w ]+)/$',
            DefaultCustomDashboard, name='project_status'),
