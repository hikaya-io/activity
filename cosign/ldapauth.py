...
class RemoteUserBackend(object):
    ...
    def authenticate(self, username=None, password=None):
        ...
        is_authenticated = self.authenticate_user(
            username, password, ldap_info['email'], ldap_info['dn'])
        is_member = ldap_info['member']
        is_admin = ldap_info['admin']
        logger.error("is_authenticated %r    is_member: %r" %
                     (is_authenticated, is_member))
    ...
    def update_userprofile(self, user, user_info):
        ...
        if created:
            ...
            logger.info("Updating record for userprofile %s" %
                        user_info['full_name'])
    ...
    def create_or_sync_ldap_user(self, ldap_info, password):
        ...
        if created:
            logger.info("AFTER: Created new user: %s created: %r" %
                        (ldap_info['full_name'], created))
        else:
            logger.info("Existing user: %s created: %r" %
                        (ldap_info['full_name'], created))
    ...
    def get_ldap_info(self, attr, value):
        ...
        try:
            retrieve_attributes = [
                'uid', 'mail', 'employeeNumber', 'givenName', 'sn', 'cn', 'dn', 'nsAccountLock']
            search_filter = attr + '=' + value
            l = ldap.initialize(settings.LDAP_SERVER)
            l.simple_bind_s(settings.LDAP_LOGIN, settings.LDAP_PASSWORD)
            r = l.search_s(base_dn, search_scope,
