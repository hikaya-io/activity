...
function formatDate(dateString, day = 0) {
	...
	try {
		...
		var ret = month
			.concat(' ')
			.concat(day == 0 ? dateval.getDate() : day)
			.concat(', ')
			.concat(dateval.getFullYear());
		return ret;
	} catch (err) {
		console.log(err);
		try {
	...
	$(whereToAppend).append(
		$(
			"<div class='alert alert-" +
				type +
				" dynamic-alert alert-dismissable' style='margin-top:0;'>" +
...
$(document).ready(function() {
	...
	$('#services').change(function() {
		...
		if (typeof services_dropdwon_has_changed != 'undefined')
			services_dropdwon_has_changed(selected_service);
	});
	...
	$('select#id_country').change(function() {
		...
		// page-specific-action call if a page has implemented the 'country_dropdwon_has_changed' function
		if (typeof country_dropdown_has_changed != 'undefined')
			country_dropdown_has_changed(selected_country);
	});
	...
	$('select#id_district').change(function(vent) {
		...
		if (typeof district_dropdown_has_changed != 'undefined')
			distirct_dropdown_has_changed(district_office);
	});
...
function newPopup(url, windowName) {
	return window.open(
		url,
		windowName,
		'height=768,width=1366,left=1200,top=10,titlebar=no,toolbar=no,menubar=no,location=no,directories=no,status=no'
	);
}
